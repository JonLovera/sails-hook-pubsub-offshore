  ...
  return {
    ...
    initialize: function(cb) {
      ...
      if (!sails.hooks.sockets) {
        return cb( Err.dependency('', 'sockets') );
      }


      if (!sails.hooks['orm-offshore']) {
        return cb( Err.dependency('', 'orm-offshore') );
      }
      ...
      // When the orm-offshore is reloaded, re-apply all of the  methods to the
      // models
      sails.on('hook:orm-offshore:reloaded', function() {
        self.augmentModels();

        // Trigger an event in case something needs to respond to the  reload
        sails.emit('hook::reloaded');
      });
    ...
    augmentModels: function() {
      ...
      for (var identity in sails.models) {
        var AugmentedModel = _.defaults(sails.models[identity], getMethods(), {autosubscribe: true} );
  ...
  function getMethods () {
    ...
    return {
      ...
      publish: function (models, eventName, context, data, req) {
        ...
        if (!models) {
          STRINGFILE.logDeprecationNotice(
            'Model.publish(null, ...)',
              STRINGFILE.get('links.docs.sockets.pubsub-offshore'),
              sails.log.debug) &&
      ...
      classRoom: function () {
        STRINGFILE.logDeprecationNotice(
          'Model.classRoom',
            STRINGFILE.get('links.docs.sockets.'),
            sails.log.debug) &&
        STRINGFILE.logUpgradeNotice(STRINGFILE.get('upgrade.classrooms'), [], sails.log.debug);
