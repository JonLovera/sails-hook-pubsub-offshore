...
module.exports = function(sails) {
  ...
  return {
    ...
    initialize: function(cb) {
      ...
      sails.on('hook:orm-offshore:loaded', function() {
      ...
      // When the orm-offshore is reloaded, re-apply all of the pubsub methods to the
      // models
      sails.on('hook:orm-offshore:reloaded', function() {
        self.augmentModels();
    ...
    return {
      ...
      publish: function(ids, data, req) {
        ...
        if (!_.isArray(ids) || _.any(ids, function(id) {return !_.isString(id) && !_.isNumber(id);})) {
          ...
          sails.log.warn('The first argument passed to `' + self.identity + '.publish()` must be an array of ids.  To subscribe to a single record, wrap the id in an array.');
      ...
      */
      subscribe: function (req, ids) {
        ...
        if (!socket) {
          sails.log.warn('`Model.subscribe()` called by a non-socket request. Only requests originating from a connected socket may be subscribed. Ignoring...');
          return;
        }
      ...
      unsubscribe: function (req, ids) {
        ...
        if (!socket) {
          sails.log.warn('`Model.unsubscribe()` called by a non-socket request. Only requests originating from a connected socket may be subscribed. Ignoring...');
          return;
        }
        ...
        if (!_.isArray(ids) || _.any(ids, function(id) {return !_.isString(id) && !_.isNumber(id);})) {
          ids = _.pluck(pluralize.apply(this, [ids]), this.primaryKey);
          sails.log.warn('The first argument passed to `' + self.identity + '.unsubscribe()` must be an array of ids.  To subscribe to a single record, wrap the id in an array.');
          sails.log.warn('For example: `[' + ids[0] + ']`');
          sails.log.warn('Wrapping it in an array for you this time...');
        }
              ...
              if (reverseAssociation) {
                _.each(previous[association.alias], function(associatedModel) {
                  ...
                  // If it's a to-many, publish a "removed" alert
                    ...
                    ReferencedModel._publishRemove(associatedModel[ReferencedModel.primaryKey], reverseAssociation.alias, id, req, {noReverse:true});
                  }
                });
              }
            }
      ...
      _watch: function ( req ) {
        ...
        if (!socket) {
          return sails.log.warn('`Model._watch()` called by a non-socket request. Only requests originating from a connected socket may be subscribed. Ignoring...');
        }
